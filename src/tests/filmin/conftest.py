...
from filmin.infra.database.sqlalchemy.models.production_company import production_company
...
async def clean_production_company(async_session_maker: AsyncSessionCtxT) -> AsyncGenerator:
    yield
    async with async_session_maker() as session:
        stmt = production_company.delete()
        await session.execute(stmt)
...
@pytest_asyncio.fixture
async def production_company1(async_session_maker: AsyncSessionCtxT) -> dict[str, str]:
    async with async_session_maker() as session:
        name = fake.company()
        stmt = production_company.insert().values(name=name).returning(production_company.c.uuid)
        result = await session.execute(stmt)
        await session.commit()
        return {'uuid': str(result.scalar_one()), 'name': name}


@pytest_asyncio.fixture
async def production_company2(async_session_maker: AsyncSessionCtxT) -> dict[str, str]:
    async with async_session_maker() as session:
        name = fake.company()
        stmt = production_company.insert().values(name=name).returning(production_company.c.uuid)
        result = await session.execute(stmt)
